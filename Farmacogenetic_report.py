...
# Dict van genen en de variaties van die genen waar Nifgo hun uitslagen op baseert.
getesteVariatiesDict = {
"CACNA1S":"WT,c.520C>T,c.3257G>A",
"CFTR":"WT, F508delCTT en G551D",
"COMT":"Val en Met",
"CYP1A2":"*1(A,F)",
"CYP2A6":"*1,*9", #Alleen *1, *9 voor dit rapport ("*1, *1H, *2, *6, *9, *11, *13, *14, *17, *20, *22, *25 t/m*28, *35(A,B), *41, *44, *4")
...
class FarmacoGeneticReport:
    def __init__(self, customer_id, doc_type):
        self.DoFu = util.DocumentFormatting(customer_id, doc_type)
    ...
    def id_table(self):
        ...
        run = paragraph.add_run(self.customer_id)
        run.font.name = "Calibri"
        run.font.size = Pt(12)
    ...
    def block_farmaco(self):
        ...
        self.DoFu.heading("De uitslag van het farmacogenetische onderzoek:", lined=True)
        paragraph = self.document.add_paragraph()
        self.DoFu.linepacing(paragraph, 0.75)
        run = paragraph.add_run("(rood: afwijkend van normaal gemiddelde fenotype/functie/acetylatie en expressie)")
    ...
    def dataframe_to_farmaco_table(self, df, style="Table Grid"):
        ...
        for j in range(df.shape[-1]):
            ...
            run.font.size = Pt(11)
            run.font.name = 'Calibri'
        # Add rest of dataframe
        # All divergent phenotypes are marked in red
        normal_phenotypes = ["NM", "NF", "RA", "negatief", "non-expressor"]
        pm_normal_genes = ["F2", "F5"]
        blacklisted_genes = ["UGT2B7", "GSTM1",
                             "Sult1E1"]  # "F2", "F5", "MTHFR1298", "MTHFR677", (As per 1 August 2023, these genes are in the rapport, not the Tromboseprofiel)
        ...
        for i in range(df.shape[0]):
            normal_check = util.common_data(list(df.iloc[i]), normal_phenotypes)
            blacklist_check = util.common_data(list(df.iloc[i]), blacklisted_genes)
            if blacklist_check:
                continue
            else:
                table.add_row()
                for j in range(df.shape[-1]):
                    if normal_check or pm_normal_check(i):
                        self.DoFu.styled_cell_text(table.cell(counter, j), str(df.iloc[i, j]))
                    else:
                        if util.common_data(list(df.iloc[i]), ["F5",
                                                             "F2"]) and j == 1:  # Dit stuk maakt het afwijkende fenotype van F2 of F5 "risico", want het fenotype aanpassen naar risico raakt teveel dingen.
                            self.DoFu.styled_cell_text(table.cell(counter, j), "risico")
                        else:  # Anders wordt gewoon het fenotype uit het dataframe gebruikt.
                            self.DoFu.styled_cell_text(table.cell(counter, j), str(df.iloc[i, j]))
                        self.DoFu.change_table_row(table.rows[counter], font_color="FF0000")
            counter += 1


    def block_2(self):
        self.DoFu.heading("\nVerklaring aanduiding fenotype/functie", chosen_size=11, colour=(68, 84, 106), lined=True)
        ...
        verklaring_text = ["UM", ": (ultra-rapid) ultra snel metabolisme, ", "RM", ": (rapid) versneld metabolisme;\n",
                           "NM", ": (normal) normaal metabolisme, ", "IM", ": (intermediate) verminderd metabolisme;\n",
                           "PM", ": (poor) nauwelijks of geen metabolisme;"]
        ...
        run.font.size = Pt(12)
        run.font.name = 'Calibri'
        self.DoFu.heading("De testuitslagen op pagina 2 hebben betrekking op de volgende genen:", lined=True)
        ...
        self.DoFu.heading("Variaties waarop is getest", lined=True)
        testvariaties_table = self.document.add_table(rows=len(getesteVariatiesDict) + 1, cols=2, style="Table Grid")
        self.DoFu.styled_cell_text(testvariaties_table.cell(0, 0), "Gen", make_bold=True, set_linespacing=True)
        self.DoFu.styled_cell_text(testvariaties_table.cell(0, 1), "Variaties", make_bold=True, set_linespacing=True)
        for i in range(2):
            if i == 0:
                for j, k in zip(range(len(getesteVariatiesDict)), getesteVariatiesDict.keys()):
                    self.DoFu.styled_cell_text(testvariaties_table.cell(j + 1, i), text=k, set_linespacing=True)
            else:
